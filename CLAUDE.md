...
`tools/run-mutation-check` exists to prove the security tests are not vacuous:
removing a guard must break a test.
...
(`.github/workflows/`: `R-CMD-check`, `hardening`, `native-checks`, `coverage`,
`pkgdown`).
