...
where the archive holds the Expat implementation and no R code. `R_ARCH` is
empty on most platforms, so the archive is usually in `zuxml/lib`. `LinkingTo:
zuxml` puts the headers on the include path. A `configure` script finds the
archive, asking for `lib/<arch>` first and falling back to `lib`, and writes
its path into `src/Makevars` without adding an `Imports:` dependency:
