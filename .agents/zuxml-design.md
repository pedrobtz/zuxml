...
The connection API is declared unstable by R, so the source `#error`s unless `R_CONNECTIONS_VERSION` is 1. The R side follows `readBin()`'s convention: an unopened connection is opened in `"rb"` and closed on exit, an open one must already be binary and is left open at wherever the parser stopped. The C side additionally refuses a text-mode or non-blocking connection, because on a non-blocking one a zero-byte read means "nothing yet", not end of input, and there is no way to tell the two apart. Base R's `url()` covers `http`, `https` and `ftp` with no dependency; `zuhttp`'s `resp_xml()` remains the path for anything needing headers, retries or a `charset` from the response.
