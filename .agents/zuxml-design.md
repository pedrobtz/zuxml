...
Independently of `zuhttp`, `xml_read()` accepts a URL string or any R connection and streams it through R's connection API (`R_ext/Connections.h`: `R_GetConnection`, `R_ReadConnection`, the same use iotools makes of it). The mechanism is format-agnostic and lives in two files meant to be copied verbatim into sibling packages (zuhtml, zujson, zuyaml): `src/zu_source.h`, a header-only byte source with a pull call (`zu_source_read`) and a push pump (`zu_source_pump`) over either a raw vector or a connection, and `R/zu_source.R`, which resolves a path, URL or connection to an open binary connection (`zu_open_input`) and reads one whole (`zu_read_all`). Each carries an origin line naming the package and version it was copied from. What stays in `r_document.c` is the sink (`zux_tree_feed`), the `R_UnwindProtect` wrapper and its cleanup, and the transcoding decision; `tests/testthat/test-source.R` is the source's own suite, written against `read_input <- xml_read` so a copy changes one line. A string with an `http`, `https`, `ftp`, `ftps` or `file` scheme is opened with `url()`, any other string with `file()`; the scheme is lower-cased first because `url()` is case-sensitive where RFC 3986 is not. The C entry point reads 64 KiB at a time into the same `zux_tree_feed` loop the raw-vector path uses, with `R_CheckUserInterrupt()` between reads and the whole thing under `R_UnwindProtect`, so an R error raised from inside `R_ReadConnection()` (a dropped network connection) releases the in-flight tree. The body is never held whole; the limits bound the tree.

**API status, and why the read is in C.** `R_GetConnection` and `R_ReadConnection` are *experimental API* on current R: R 4.6's Writing R Extensions lists them in its "Experimental API index", and R 4.6 dropped them from the non-API table `R CMD check` consults, where R 4.5 still has them. So R 4.5's check reports them as non-API calls (seen first on zuhtml, then on this package's oldrel job), and R 4.6 and later do not. CRAN's incoming check runs on R-devel, where the package is clean; the NOTE appears only on CRAN's oldrel flavours, which does not by itself cause a rejection, and `cran-comments.md` explains it. A revision of #68 moved the read into an R-level `readBin()` loop over begin/feed/end entry points to avoid the NOTE entirely; that was reverted once the API status was established, because the C source is the simpler contract for sibling packages to copy and the NOTE is version-bound, not policy. The header-only C source `#error`s unless `R_CONNECTIONS_VERSION` is 1, since experimental means R reserves the right to change the connection implementation without a compatibility layer. The R side follows `readBin()`'s convention: an unopened connection is opened in `"rb"` and closed on exit, an open one must already be binary and is left open at wherever the parser stopped. The C side additionally refuses a text-mode or non-blocking connection, because on a non-blocking one a zero-byte read means "nothing yet", not end of input, and there is no way to tell the two apart. Base R's `url()` covers `http`, `https` and `ftp` with no dependency; `zuhttp`'s `resp_xml()` remains the path for anything needing headers, retries or a `charset` from the response.
