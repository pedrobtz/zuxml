...
| 8 | Round-trip (parse → serialize → parse) is structurally identical across the corpus | `test-serialize.R`, whose corpus includes the whitespace character references (`&#13;`, `&#13;&#10;`, `&#9;`) that survive end-of-line normalization; the `fuzz_roundtrip` target aborts on any non-fixed-point, survived 2.5M inputs, and `fuzz/corpus/seed21.xml` seeds it with those cases |
